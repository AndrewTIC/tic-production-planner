...
**Admin corrections.** Admins can adjust, add, or delete clockings to fix mis-clocks, wrong-BU selections, and forgotten clock-ons. Every correction retains the original values and records who changed what and when. **Deletion is a soft-delete**: the UI requires a clear confirmation warning before deleting; the entry is marked voided rather than removed, keeping its original values and recording who voided it and when. Voided entries are excluded from all totals and all screens except an Admin audit view, from which an entry voided in error can be restored. Hard deletion is impossible for every role, database-enforced. During shopfloor trials the workspace is shared and clockings may be **wholly managed at admin level** (admins entering time on workers' behalf) until the tool is deployed for direct use — the interface supports both modes without configuration.
...
| `time_entries` | operation_id, worker_id, started_at, ended_at, ot_class (none/1.5/2.0, derived), auto_closed, adjusted_by (nullable), adjusted_at, original_values (jsonb), voided (bool), voided_by, voided_at | OT class derived from times/day; corrections audited in place; admin delete = void (soft-delete), excluded everywhere except the Admin audit view |
